...
class Paths( object ):
	...
	def get_frame_path_sequences( self, frame ):
		# validate frame
		frame %= 3
		
		frame_path_sequences = list()
		for p in self.all_path_sequences:
			if p[0][0] == frame:
				frame_path_sequences.append( p )
			elif p[1][0] == frame:
				frame_path_sequences.append( p[1:] )
		
		return frame_path_sequences