...
#		if frame % 3 != 0:
	...
	def dist_to_stop( self, frame, pos=0 ):
		...
		found_stop = False
		reached_end = False
		while not found_stop and not reached_end:
			try:
...
class BiologicalSequence( RandomFSSequence ):
	...
	def detect_frameshifts( self, frame=0 ):
		...
		print "frame\tpos\tscore"
		print "%d\t%d\t%d" % ( frame, pos, score )

		while pos < matrix_length:
			f, l, s = self.get_best_frame( frame, pos )	# frame, length, score
			frame = f
			pos += l + 1
			score += s
			print "%d\t%d\t%d" % ( frame, pos, score )
